...
## 十三、续查 VII：白歌的阵容与策略（2026-09-13）

任务：找白歌（player-baige）对应的阵容与策略并记录。

来源汇总：
- 棋手机制：`chessplayers.json`（wiki chessplayerFacts，官网棋手技能原文）。
- 阵容：官网首页新人推荐（白歌→三分登场流）、官方新手指引入门阵容、wiki 25 套预设（白歌三分倒转、白歌高阶逐鹿）、大神教学（西君《白歌三分倒转！全员999强力阵容》）。

### 13.1 白歌技能链（官方）

神来之笔（每 4 次合成→1 张即兴创作）→ 即兴创作（随机复制场上 1 名英雄）→ 天生我才（6 张即兴创作→灵感爆发）→ 灵感爆发（斩手牌 3 张非觉醒各为 2 张转瞬复制）→ 惊才绝艳/奇才（指定 1 名英雄获得 4 张牌）。
策略本质：合成是唯一资源入口，复制滚雪球；奇才定点 4 张核心；灵感爆发翻倍手牌。

### 13.2 白歌关联 3 套阵容

1. **三分登场流**（官网推荐白歌，官方新手阵容，代表英雄吕布）：三分之地「登场」触发链（吕布/周瑜/小乔/甄姬/曹操），低阶养成、适合新手。
2. **白歌三分倒转**（wiki + 西君大神教学）：early 7 人（孙悟空/周瑜/曹操/小乔/阿轲/甄姬/露娜），登场链 + 白歌复制滚到全员 999；core_idea「怎么活到四级D到甄姬小乔」；mid/late 污染不采用。
3. **白歌高阶逐鹿**（wiki）：early 7 人（白起/西施/嬴政/庄周/芈月/蒙犽/鲁班七号），逐鹿战术牌流堆嬴政法攻；core_idea「怎么活到四级D到嬴政」；mid/late 污染不采用。

### 13.3 冲突与未知

- 冲突：白歌三分倒转 mid/late 与多套共用站位（污染）→ 只用 early+标题+core_idea。
- 未知：白歌两套 wiki 阵容无官方分阶段运营；「倒转」等级滚动上限/顺序无官方数值。
- 白歌技能卡（即兴创作/灵感爆发/奇才）不在效果牌目录（棋手专属牌）。

### 13.4 本轮改动清单（续）

- 新增 `json/chessplayer_lineups.json`（棋手→阵容查表，白歌首条）
- 更新 `markdown/02-lineups.md`（白歌的阵容与策略节）、`markdown/00-INDEX.md`
